...
const { logSignal, getReport } = require('./monitor');
...
let freeCount = 0;
let vipCount = 0;
const MAX_FREE = 20; // 15-20 como combinamos
const MAX_VIP = 15; // 8-15 como combinamos
...
async function analyzePair(pair) {
  ...
  let vipSignal = await gerarSinalVip({symbol: pair, exchange: "BINANCE"}); // Integra main_vip.js
  if (vipSignal.ok) {
    vipCount++;
    if (vipCount <= MAX_VIP) {
      const formatted = formatSignal(vipSignal.payload, 'VIP');
      await sendToTelegram(VIP_CHANNEL_ID, formatted);
      if (DISCORD_TOKEN) await sendToDiscord(DISCORD_VIP_CHANNEL_ID, formatted);
      logSignal(vipSignal.payload, 'VIP');
      console.log(`💎 VIP ENVIADO (${vipCount}/${MAX_VIP}) | ${pair}`);
    }
  }
  let freeSignal = await gerarSinalFree({symbol: pair, exchange: "BINANCE"}); // Integra main.js
  if (freeSignal.ok) {
    freeCount++;
    if (freeCount <= MAX_FREE) {
      const formatted = formatSignal(freeSignal.payload, 'FREE');
      await sendToTelegram(FREE_CHANNEL_ID, formatted);
      logSignal(freeSignal.payload, 'FREE');
